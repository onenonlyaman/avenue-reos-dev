...
export function CommunicationsApprovalDrawer({ isOpen, onClose, onRefresh }: CommunicationsApprovalDrawerProps) {
  const [approvals, setApprovals] = useState<CommunicationsApprovalItem[]>([]);
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await communicationsApi.getPendingApprovals();
      setApprovals(data);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Pending ticket approvals could not be loaded");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAuthorize = async (id: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Dispute settlement could not be authorized");
    } finally {
  ...
  const handleReject = async (id: string) => {
    try {
      setProcessingId(id);
      await communicationsApi.rejectApproval(id, "Rejected by Customer Governance Director");
      await loadData();
      onRefresh();
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Dispute settlement could not be rejected");
    } finally {
      setProcessingId(null);
    }
  };
      ...
      <SheetContent className="w-full sm:max-w-xl border-border bg-card p-6 overflow-y-auto">
        <SheetHeader className="pb-4 border-b border-border space-y-1">
          <SheetTitle className="text-base font-bold text-foreground flex items-center gap-2">
            ...
            Customer Governance Director Escalation Queue
          </SheetTitle>
          <SheetDescription className="text-xs text-muted-foreground">
            Human-in-the-Loop executive review for buyer financial claim disputes (&gt; ₹1 Lakh) and legal notice threat escalations.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="py-4 space-y-4">
          {isLoading ? (
            ...
            <CorporateEmptyState
              title="No Pending Ticket Escalations"
              description="All high-risk customer disputes and legal threat tickets have been reviewed and resolved."
                ...
                <div key={item.id} className="border border-border rounded-lg p-4 bg-muted/20 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-1">{item.issueSummary}</p>
                    </div>
