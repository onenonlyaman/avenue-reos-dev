...
export function IntegrationsApprovalDrawer({ isOpen, onClose, onRefresh }: IntegrationsApprovalDrawerProps) {
  const [approvals, setApprovals] = useState<IntegrationsApprovalItem[]>([]);
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await integrationsApi.getPendingApprovals();
      setApprovals(data);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Pending integration approvals could not be loaded");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAuthorize = async (id: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Integration execution could not be authorized");
    } finally {
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Integration execution could not be rejected");
    } finally {
      setProcessingId(null);
    }
  };
      ...
      <SheetContent className="w-full sm:max-w-xl border-border bg-card p-6 overflow-y-auto">
        <SheetHeader className="pb-4 border-b border-border space-y-1">
          <SheetTitle className="text-base font-bold text-foreground flex items-center gap-2">
            ...
            Governance Director Integration Safeguard Queue
          </SheetTitle>
          <SheetDescription className="text-xs text-muted-foreground">
            Human-in-the-Loop executive authorization for high-value ERP ledger syncs (&gt; ₹10 Lakhs), gateway refunds (&gt; ₹50,000), and API vault key rotations.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="py-4 space-y-4">
          {isLoading ? (
            ...
            <CorporateEmptyState
              title="No Pending Integration Safeguards"
              description="All high-value ERP sync operations, gateway refunds, and credential rotations have been reviewed and authorized."
                ...
                <div key={item.id} className="border border-border rounded-lg p-4 bg-muted/20 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-foreground">{item.connectorName}</span>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      {item.syncAmount > 0 && (
                        <span className="text-sm font-mono font-extrabold text-primary">
                          {formatCurrency(item.syncAmount)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    ...
                    >
                      ...
                      Authorize Integration Execution
                    </Button>
                  </div>
                </div>
              ))}
