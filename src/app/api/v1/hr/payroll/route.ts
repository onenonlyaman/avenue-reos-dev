...
export async function POST(request: NextRequest) {
  try {
    ...
    const gross = Number(totalGrossSalary || 1250000);
    const net = Number(netPayable || 1080000);
    ...
    const inserted = await prisma.$queryRaw<any[]>`
      INSERT INTO hr_payroll_runs (
        ...
        ${tenantId}::uuid, ${cycleMonth || "August 2026"}, ${gross}, ${pf},
        ${esic}, ${pt}, 45000, ${net}, ${status}, ${requiresHitl}, 42
      )
      RETURNING *
    `;
    ...
    if (requiresHitl) {
      ...
      await prisma.$executeRaw`
        INSERT INTO hr_approvals (
          tenant_id, type, reference_name, amount, justification, requested_by, status, requires_hitl
        ) VALUES (
          ${tenantId}::uuid, 'PAYROLL_RUN', ${`Payroll Batch - ${cycleMonth || "August 2026"}`},
          ${net}, 'Net payroll batch disbursement exceeds ₹10 Lakhs threshold requirement',
          'HR Operations Manager', 'PENDING_APPROVAL', true
        )
      `;
    }
