...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 3. Update the specific linked source record
      ...
      await tx.$executeRaw`
        INSERT INTO audit_trail_logs (
          ...
          ${tenantId}::uuid, ${officerName}, 'HR & People Operations', 'AUTHORIZE',
          ${`Authorized HR approval item: ${item.reference_name} (Amount: INR ${Number(item.amount || 0).toLocaleString("en-IN")})`},
          '127.0.0.1', true
        )
      `;
    });
